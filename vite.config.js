...
export default defineConfig(({ mode }) => {
  ...
  const requestedModel = (env.CLAUDE_MODEL || "").trim();
  const defaultModel = "claude-sonnet-4-20250514";
  const modelLooksValid = requestedModel.startsWith("claude-");
  const model = modelLooksValid ? requestedModel : defaultModel;

  return {
    plugins: [react(), tailwindcss(), workoutApiPlugin(apiKey, model)],
  };
});
